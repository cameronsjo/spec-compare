...
- **MySpec tool profile** (`site/src/data/tools/myspec.json`, `docs/gaps.md`) — Emerging-tier profile for MySpec (myspec.dev): an interactive AI spec discovery platform that conducts guided developer interviews to compile deterministic 4-file spec bundles (`constitution.md`, `requirements.md`, `solution.md`, `tasks.md`) with official Model Context Protocol (MCP) server integration for Cursor, Claude Code, and Windsurf. Heatmap and matrices now span **21 tools, 8 core**; `data.test.ts` count invariant updated
