...
- **Heatmap detail popover + graphical modes** (`site/`) — The scoring heatmap gained a mobile-friendly detail popover (hover/focus on desktop, tap → bottom sheet on touch) and a live mode toggle to compare three graphical treatments of the same grid: numbers, color wash, and a size-encoded punchcard
