...
- **Matt Pocock's Skills profile** (`site/src/data/tools/mattpocock-skills.json`, `docs/gaps.md`) — Emerging-tier profile for `mattpocock/skills` (v1.2.3, MIT, ~268K stars), requested in #51: a composable skill collection (grill → spec → tickets → TDD, plus `/wayfinder` for multi-session work) that positions itself against process-owning frameworks. Scores come from reading the repository; it stays emerging, with no hands-on evaluation planned. Heatmap and matrices now span **20 tools, 8 core**; `data.test.ts` count invariant updated

- **SpecBuddy watch entry** (`docs/gaps.md`) — Marginal watch-list entry for SpecBuddy, a JetBrains/VS Code plugin pair that gates agent execution step by step: Markdown spec → approved plan → one step at a time in an isolated git worktree, with a per-step diff to accept, refine, or roll back (author-submitted inclusion PR #50, below the adoption bar per the #46 / #12 precedent). Numbers re-verified 2026-09-22: usage grew since submission (448 JetBrains downloads, 16 VS Code installs), and the Open VSX listing cited at submission no longer resolves. Closed source, so the claims trace to the marketplace listings rather than to inspectable source; revisit for an emerging-tier profile when adoption clears the bar
...
- **Heatmap tool column at phone width** (`site/src/styles.css`) — The sticky tool column was content-sized to ~310px (longest name), leaving the score dots past the fold with only a sliver peeking after a swipe. At ≤640px the header's inner name/version spans now cap at 104px with ellipsis (heatmap only — the matrix card view keeps full names), so ~2 score columns are visible before any swipe
...
- **Analytics disclosure** (`site/src/Disclosure.tsx`) — Fine-print note in the Disclosure view: Cloudflare Web Analytics is cookieless and aggregate-only; GitHub Pages logs visitor IPs per GitHub's privacy statement. No consent banner needed (no device storage), added for transparency
